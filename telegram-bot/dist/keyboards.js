...
export function resultsKeyboard(hasNext, mode) {
    const rows = [];
    if (hasNext)
        rows.push([Markup.button.callback('Следующие 5 →', `${mode}:next`)]);
    if (mode === 'search')
        rows.push([Markup.button.callback('🔄 Новый поиск', 'menu:search')]);
    if (mode === 'favorites')
        rows.push([Markup.button.callback('📋 Каталог', 'menu:catalog')]);
    rows.push([Markup.button.callback('🏠 Главное меню', 'menu:start')]);
    return Markup.inlineKeyboard(rows);
}
export const requireSubscriptionKeyboard = (channel) => Markup.inlineKeyboard([
