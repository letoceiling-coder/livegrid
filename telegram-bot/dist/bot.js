...
import { contactsKeyboard, mainMenuKeyboard, requireSubscriptionKeyboard, resultsKeyboard, searchBudgetKeyboard, searchCityKeyboard, searchTypeKeyboard, } from './keyboards.js';
...
async function guardSubscribed(ctx) {
    ...
    await ctx.reply(`Чтобы пользоваться ботом, подпишитесь на наш канал:\n${config.REQUIRED_CHANNEL_ID}\n\nТам — новые ЖК, акции и полезные материалы.`, requireSubscriptionKeyboard(config.REQUIRED_CHANNEL_ID));
    return false;
}
...
async function showFavorites(ctx) {
    ...
    if (!session.jwt) {
        await ctx.reply('Чтобы видеть избранное, войдите на сайте:\nlivegrid.ru/login\n\nПосле входа свяжите аккаунт с ботом.', Markup.inlineKeyboard([
...
async function sendComplexesList(ctx, items, hasNext, mode) {
    if (items.length === 0) {
        await ctx.reply(mode === 'search'
            ? 'По вашему запросу ничего не найдено. Попробуйте изменить параметры поиска'
            : mode === 'favorites'
                ? 'У вас пока нет избранных объектов. Перейдите в каталог'
                : 'Ничего не найдено.', resultsKeyboard(false, mode));
        return;
    }
...
bot.action(/^search:type:(.+)$/, async (ctx) => {
    await ctx.answerCbQuery();
    const type = ctx.match[1] ?? 'apartments';
