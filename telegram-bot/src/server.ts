...
const app = express();
const pinoHttp = pinoHttpLib as unknown as (opts: { logger: typeof logger }) => express.RequestHandler;
...
bot.action(/^lead:accept:([^:]+):(.+)$/, async (ctx) => {
  ...
  const requestId = ctx.match[1] as string;
  const managerName = ctx.match[2] as string;
  try {
