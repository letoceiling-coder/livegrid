...
function formatStatus(status?: string, deadline?: string | null): string {
  if (status === 'completed') return '✅ Сдан';
  const normalizedDeadline = normalizeDeadline(deadline);
  if (!normalizedDeadline) return '🔑 Строится';
...
function normalizeDeadline(deadline?: string | null): string | null {
  ...
  if (kvartalMatch) return `Q${kvartalMatch[2]} ${kvartalMatch[1]}`;
  const altMatch = raw.match(/([1-4])\s*квартал\s*(20\d{2})/i);
  if (altMatch) return `Q${altMatch[1]} ${altMatch[2]}`;
  return raw;
}

export function renderComplexCard(item: ComplexCard): string {
